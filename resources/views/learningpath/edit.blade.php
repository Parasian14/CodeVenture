...
<head>
    ...
    <title>Edit Learning Path | Admin</title>
...
<body class="bg-body-tertiary">
    ...
    <section class="container-md">
        <h2 class="text-center">Edit Learning Path</h2>
        <form method = 'post' action="{{ route('LearningPath.update',['LearningPath'=>$lp]) }}" enctype="multipart/form-data">
            @method('patch')
            @csrf
            <div class="mb-3">
                <label for="judul" class="form-label">Nama</label>
                <input type="text" class="form-control border-black" name = "nama" value = "{{ $lp->nama }}" id="judul" aria-describedby="judulHelp">
                <div id="judulHelp" class="form-text">Nama Learning Path.</div>
            </div>
            <div class="mb-3">
                <label for="judul" class="form-label">Deskripsi</label>
                <input type="text" class="form-control border-black" name = "deskripsi" value = "{{ $lp->deskripsi }}" id="judul" aria-describedby="deskHelp">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control border-black" id="isi" name = "isi"rows="3">{{ $lp->isi }}</textarea>
            </div>
